...
void get_next_player(player player_list[NUM_PLAYERS],
...
void get_category(char currentCategory[MAX_LEN]) {
  ...
  while (flag) {
    ...
    for (int i = 0; i < NUM_CATEGORIES; i++) {
      if (strncasecmp(categories[i], currentCategory,
                      strlen(currentCategory)) == 0) {
        flag = false;
        break;
      }
    }
...
int get_difficulty_level(char *currentCategory) {
  int currentValue;
  ...
  while (flag) {
    ...
    if (sscanf(buffer, "%d", &currentValue) == 1 &&
        (currentValue == 100 || currentValue == 200 || currentValue == 300 ||
         currentValue == 400)) {
...
void ask_question(char *currentCategory, int currentValue, char *correctAnswer,
                  player player_list[NUM_PLAYERS], char *currentPlayer) {
...
int main(int argc, char *argv[]) {
  ...
  while (fgets(buffer, BUFFER_LEN, stdin) != NULL) {
    ...
    show_results(
        player_list,
        NUM_PLAYERS); // display current players, their names and scores
    ...
    currentValue = get_difficulty_level(currentCategory);

    // set the answered value for the question to true and save the answer to
    // check later
    char *correctAnswer;
    for (int i = 0; i < 16; i++) {
      if (strcmp(currentCategory, questions[i].category) == 0 &&
          currentValue == questions[i].value) {
    ...
    ask_question(currentCategory, currentValue, correctAnswer, player_list,
                 currentPlayer);
