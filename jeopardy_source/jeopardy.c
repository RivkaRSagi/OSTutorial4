...
void tokenize(char *input, char *tokens) {
    ...
    if (strncasecmp(input, what, strlen(what)) == 0 ||
        strncasecmp(input, who, strlen(who)) == 0) {
        // Skip "What is" or "Who is"
        strtok(input, " "); // "What" or "Who"
        strtok(NULL, " ");  // "is"
        char *answer = strtok(NULL, "\n");  // Get the actual answer
        strcpy(tokens, answer);  // Copy the answer to tokens
    } else {
        tokens = "\0";
    }
}
...
int main(int argc, char *argv[]) {
  ...
  while (fgets(buffer, BUFFER_LEN, stdin) != NULL) {
    ...
    while (flag) {
      ...
      while (getchar() != '\n')
        ; // Fix: Clears the buffer after scanf to prevent issues with fgets
    ...
    while (getchar() != '\n')
      ; // Fix: Clears buffer in case previous input left residual characters
